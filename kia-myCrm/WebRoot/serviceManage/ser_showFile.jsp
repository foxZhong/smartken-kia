          ...
          <sql:query var="grant" dataSource="${con}">
	            SELECT count(f_id) as fcount
                 FROM users,sys_function
                   where u_grant regexp 'C5' and u_id=?;
...
<div id="container">
	...
	<div id="main">
		...
		服务管理->服务归档
		<p>
		<html:form action="/serviceManage/services?url=ser_showFile" method="post">
  ...
  <tr class="listTitleTR">
   ...
   <c:forEach var="serRow" items="${services.rows}">
     ...
     <td>${serRow.uname}</td>
     <td>
     ${serRow.ser_pubtime }</td>
     <td>
     ${serRow.serstatus }
     </td>
     <td>
     <c:if test="${serRow.ser_status==5 }">
   <a href="ser_doFile.jsp?serID=${serRow.ser_id}">查看</a>
     </c:if>
     <c:if test="${serRow.ser_status<5 }"><a href="services.do?oper=doFileService&serID=${serRow.ser_id}">归档</a></c:if>
     </td>
  </tr>
  </c:forEach>
    <td colspan="8">
	      ...
	      <input type="hidden" name="url" value="serviceManage/ser_showFile.jsp"/>
 ...
 <c:if test="${list!=null}">
  <logic:iterate id="ser" name="list" indexId="i">
    ...
    <bean:write name="ser" property="serPubtime" format="yyyy-MM-dd"/>
     ...
     <td>
       <c:if test="${ser.serStatus==5 }">
      <a href="ser_doFile.jsp?serID=${ser.serId}">查看</a>
     </c:if>
     <c:if test="${ser.serStatus<5 }">
         <a href="services.do?oper=doFileService&serID=${ser.serId}">归档</a></c:if>
     </td>
  </tr>
    </html:form>
    </logic:iterate>
